...
type
  ...
  _anonymous_type_6 = record
    case Integer of
      ...
      1: (_integer: Int64);
      2: (_double: Double);
      3: (_string: _anonymous_type_3);
      4: (_object: _anonymous_type_4);
      5: (_array: _anonymous_type_5);
  end;
  ...
  IGVInterface = interface
    ...
    function computed_property_create(vm: Pgravity_vm;
      getter_func: Pgravity_function_t; setter_func: Pgravity_function_t)
      : Pgravity_closure_t;
    procedure computed_property_free(c: Pgravity_class_t; const name: AnsiString;
      remove_flag: Boolean);
    ...
    function VALUE_FROM_STRING(vm: Pgravity_vm; const s: AnsiString; len: UInt32)
      : gravity_value_t;
...
{$ELSE GRAVITY_STATIC_LINK}
  ...
  begin
    ...
    Fgravity_core_class_from_name := GetProcAddr('gravity_core_class_from_name');
...
function TGVInterface.VALUE_AS_BOOL(val: Pgravity_value_t): gravity_int_t;
begin
  result := val^.f2.n;
end;

function TGVInterface.VALUE_AS_CLASS(val: Pgravity_value_t): Pgravity_class_t;
begin
  result := Pgravity_class_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_CLOSURE(val: Pgravity_value_t)
  : Pgravity_closure_t;
begin
  result := Pgravity_closure_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_ERROR(val: Pgravity_value_t): PAnsiChar;
begin
  result := PAnsiChar(val^.f2.p);
end;

function TGVInterface.VALUE_AS_FIBER(val: Pgravity_value_t): Pgravity_fiber_t;
begin
  result := Pgravity_fiber_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_FLOAT(val: Pgravity_value_t): gravity_float_t;
begin
  result := val^.f2.f;
end;

function TGVInterface.VALUE_AS_FUNCTION(val: Pgravity_value_t)
  : Pgravity_function_t;
begin
  result := Pgravity_function_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_INSTANCE(val: Pgravity_value_t)
  : Pgravity_instance_t;
begin
  result := Pgravity_instance_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_INT(val: Pgravity_value_t): gravity_int_t;
begin
  result := val^.f2.n;
end;

function TGVInterface.VALUE_AS_LIST(val: Pgravity_value_t): Pgravity_list_t;
begin
  result := Pgravity_list_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_MAP(val: Pgravity_value_t): Pgravity_map_t;
begin
  result := Pgravity_map_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_OBJECT(val: Pgravity_value_t): Pgravity_object_t;
begin
  result := val^.f2.p;
end;

function TGVInterface.VALUE_AS_RANGE(val: Pgravity_value_t): Pgravity_range_t;
begin
  result := Pgravity_range_t(VALUE_AS_OBJECT(val));
end;

function TGVInterface.VALUE_AS_STRING(val: Pgravity_value_t): Pgravity_string_t;
begin
...
function TGVInterface.gravity_function_new(vm: Pgravity_vm;
  const identifier: AnsiString; nparams: UInt16; nlocals: UInt16; ntemps: UInt16;
  code: Pointer): Pgravity_function_t;
begin
...
function TGVInterface.gravity_function_new_internal(vm: Pgravity_vm;
  ...
  : Pgravity_function_t;
begin
  result := Fgravity_function_new_internal(vm, PAnsiChar(identifier), exec, nparams);
end;
...
function TGVInterface.gravity_class_new_pair(vm: Pgravity_vm;
  ...
  nsvar: UInt32): Pgravity_class_t;
begin
  result := Fgravity_class_new_pair(vm, PAnsiChar(identifier), superclass, nivar, nsvar);
end;
...
begin
  result := Fgravity_compiler_run(compiler, PAnsiChar(source), len, fileid, is_static,
    add_debug);
end;
...
begin
  result := Fgravity_compiler_serialize_infile(compiler, closure, PAnsiChar(path));
end;
...
function TGVInterface.gravity_vm_getvalue(vm: Pgravity_vm; const key: AnsiString;
  keylen: UInt32): gravity_value_t;
begin
